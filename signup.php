...
<?php
    ...
    <script>
        ...
        button.addEventListener("click", ()=>{
            fetch("./dbUtils/create.php", {method: "POST", headers: {'Content-Type': "application/json", 'Accept': "application/json"}, body: JSON.stringify({table:"Users", values:{name: name.value, pwd: pwd.value}, pk: "name"})})
            .then(e=>e.json())
            .then(e=>{console.log(e);
                    if(e.message == "user created"){
                        queryMessage.textContent = "Votre compte a bien été créé !"
                        setTimeout(()=>{queryMessage.textContent = "";window.location.href = "./login.php";}, 2000);
                        
                    }else{
                        queryMessage.textContent = "Votre compte n'a pas été créé !"
